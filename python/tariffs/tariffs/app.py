...
import proto.tariffs_pb2_grpc as tariffs_pb2_grpc
import proto.tariffs_pb2 as tariffs_pb2
...
# прототип для подписок и клиентских статистик
# хранит id тарифа (статистики), компании, и считает всё это говно
...
class TariffsServiceServ(companies_pb2_grpc.CompanyServicer):
...
def main():
    ...
    tariffs_pb2_grpc.add_TariffsServicer_to_server(TariffsServiceServ(database), server)
